...
<template>
  <div class="mm-tab-content">
    <div class="mm-tab-scroll">
      ...
      <div v-if="statusExpanded" class="dev-card" style="margin-top:2px">
      ...
      <div class="review-section">
        ...
        <span v-if="!guideExpanded" class="section-detail"><template v-if="sentimentMode==='intraday'&&intradayLatest&&!isIntradayFallback">当前<b :style="{color:scoreColor(intradayLatest.score||0)}">{{ periodCN(intradayLatest.period||'') }}</b>，建议仓位<b>{{ (intradayLatest.score||0)>=70?'100%满仓':(intradayLatest.score||0)>=55?'70%重仓':(intradayLatest.score||0)>=40?'50%半仓':'30%轻仓' }}</b>，策略：<b>{{ (intradayLatest.score||0)>=70?'积极做多':(intradayLatest.score||0)>=55?'精选龙头':(intradayLatest.score||0)>=40?'轻仓低吸':'空仓观望' }}</b></template><template v-else-if="sentimentLive">当前<b :style="{color:scoreColor(sentimentLive.score||0)}">{{ (sentimentLive.score||0)>=70?'🔥高潮期':(sentimentLive.score||0)>=55?'⚡分化期':(sentimentLive.score||0)>=40?'🌀震荡期':'🥶冰点期' }}</b>，建议仓位<b>{{ (sentimentLive.score||0)>=70?'100%满仓':(sentimentLive.score||0)>=55?'70%重仓':(sentimentLive.score||0)>=40?'50%半仓':'30%轻仓' }}</b>，策略：<b>{{ (sentimentLive.score||0)>=70?'积极做多':(sentimentLive.score||0)>=55?'精选龙头':(sentimentLive.score||0)>=40?'轻仓低吸':'空仓观望' }}</b></template></span>
      </div>
      ...
      <div class="review-section">
        ...
        <span v-if="!algoDimExpanded" class="section-detail section-oneline">8个维度加权求和得0~100分，映射为4个情绪阶段：🔥高潮(≥70) → ⚡分化(≥55) → 🌀震荡(≥40) → 🥶冰点(&lt;40)</span>
      </div>
...
.review-section { padding: 3px 8px; background: var(--bg-elevated); border: 1px solid var(--border-default); border-radius: 4px; margin-bottom: 2px; display: flex; align-items: baseline; flex-wrap: nowrap; gap: 4px; overflow: hidden; }
...
.section-detail { font-size: 11px; color: var(--text-secondary); line-height: 1.6; flex-shrink: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
