      ...
      <template v-if="reviewTab === 'monthly'">
        <template v-if="monthlyReviewData">
          ...
          <div v-if="monthlyBehaviorExpanded" class="review-2col" style="margin-top:2px">
            <div class="dev-card"><div class="dev-title">🛡️ 止损执行率</div><div class="dev-row"><span>执行率(止损覆盖亏损)</span><span :class="monthlyReviewData.behavior_drift?.stop_loss_execution_rate >= 90 ? 'up' : 'down'">{{ monthlyReviewData.behavior_drift?.stop_loss_execution_rate || 0 }}%</span></div><div class="dev-row" style="font-size:11px;color:var(--text-tertiary)"><span>亏损走止损{{ monthlyReviewData.behavior_drift?.stop_loss_at_loss || 0 }}笔 / 止损触发{{ monthlyReviewData.behavior_drift?.stop_loss_triggered || 0 }}笔 / 全部亏损{{ monthlyReviewData.behavior_drift?.loss_sells || 0 }}笔</span></div><div v-if="monthlyReviewData.behavior_drift?.stop_loss_triggered_rate" class="dev-row"><span>止损命中(触发中真亏损)</span><span>{{ monthlyReviewData.behavior_drift?.stop_loss_triggered_rate || 0 }}%</span></div></div>
