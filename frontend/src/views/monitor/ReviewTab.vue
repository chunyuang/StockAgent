      ...
      <template v-if="reviewTab === 'monthly'">
        <template v-if="monthlyReviewData">
          ...
          <div class="review-2col">
            <div class="dev-card"><div class="dev-title">🛡️ 止损执行率</div><div class="dev-row"><span>已执行止损/止损触发</span><span :class="monthlyReviewData.behavior_drift?.stop_loss_execution_rate >= 90 ? 'up' : 'down'">{{ monthlyReviewData.behavior_drift?.stop_loss_execution_rate || 0 }}%</span></div><div class="dev-row" style="font-size:11px;color:var(--text-tertiary)"><span>止损执行{{ monthlyReviewData.behavior_drift?.stop_loss_executed || monthlyReviewData.behavior_drift?.stop_loss_at_loss || 0 }}笔 / 触发{{ monthlyReviewData.behavior_drift?.stop_loss_triggered || monthlyReviewData.behavior_drift?.total_stop_loss || 0 }}笔</span></div></div>
...
<ElDialog :model-value="dailyReportVisible" @update:model-value="emit('update:dailyReportVisible', $event)" title="📈 每日复盘报告" width="750px">
  <div v-if="dailyReport" class="dr">
    <div class="dr-sec"><div class="dr-t">💰 账户概览</div><div class="dr-g"><div class="dr-i"><span class="dr-l">总资产</span><span class="dr-v">{{ ((dailyReport?.account?.total_assets || 0) / 10000).toFixed(1) }}万</span></div><div class="dr-i"><span class="dr-l">可用</span><span class="dr-v">{{ ((dailyReport?.account?.available_cash || 0) / 10000).toFixed(1) }}万</span></div><div class="dr-i"><span class="dr-l">仓位</span><span class="dr-v">{{ dailyReport?.account?.position_ratio || 0 }}%</span></div><div class="dr-i"><span class="dr-l">今日盈亏</span><span class="dr-v" :class="(dailyReport?.account?.today_profit || 0) >= 0 ? 'up' : 'down'">{{ (dailyReport?.account?.today_profit || 0) >= 0 ? '+' : '' }}{{ (dailyReport?.account?.today_profit || 0).toFixed(0) }}</span></div></div></div>
