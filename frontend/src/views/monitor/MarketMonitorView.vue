...
const debugSignals = [
  { _debug_signal: true, ts_code: '002777.SZ', stock_name: '久远银海', strategy: 'halfway_chase', strategy_name: '半路追涨', signal_status: 'preview', pct_chg: 5.8, volume_ratio: 2.6, turnover_rate: 8.4, reason: '调试样例：放量突破均线，涨幅处于半路追涨区间' },
  { _debug_signal: true, ts_code: '603598.SH', stock_name: '引力传媒', strategy: 'first_limit_up', strategy_name: '首板打板', signal_status: 'preview', pct_chg: 10.0, volume_ratio: 3.1, turnover_rate: 12.7, reason: '调试样例：首板封单较强，换手充分' },
  { _debug_signal: true, ts_code: '000517.SZ', stock_name: '荣安地产', strategy: 'limit_up_open', strategy_name: '涨停开板', signal_status: 'preview', pct_chg: 9.6, volume_ratio: 2.2, turnover_rate: 10.3, reason: '调试样例：涨停开板后回封观察，展示历史/调试信号样式' },
]
const visibleSignals = computed(() => filteredSignals.value.length ? filteredSignals.value : debugSignals.filter((s: any) => signalFilter.value === 'all' || s.strategy === signalFilter.value || (signalFilter.value === 'anomaly' && String(s.strategy).startsWith('anomaly_'))))
  ...
  <div class="mm" :class="{ dark: themeStore.isDark }">
    ...
    <div v-if="activeTab === 'trading'" class="mm-body" :class="{ 'left-expanded': leftPanelExpanded, 'position-expanded': anyPositionExpanded, 'rail-collapsed': leftRailCollapsed }">
      ...
      <div class="mm-right signals-right">
        <div class="st">🎯 {{ visibleSignals.some((s: any) => s._debug_signal) ? '调试信号' : visibleSignals.some((s: any) => s._historical_signal) ? '历史信号' : '活跃信号' }} <ElTag v-if="visibleSignals.some((s: any) => s._debug_signal)" size="small" type="warning" style="margin-left:4px">样例</ElTag><ElTag v-else-if="visibleSignals.some((s: any) => s._historical_signal)" size="small" type="info" style="margin-left:4px">最近留存</ElTag> <div style="display:inline-flex;gap:2px;margin-left:6px"><ElTag v-for="f in [{k:'all',l:'全部'},{k:'halfway_chase',l:'半路'},{k:'first_limit_up',l:'首板'},{k:'limit_down_qiao',l:'跌停'},{k:'anomaly',l:'异动'}]" :key="f.k" size="small" :type="signalFilter===f.k?'primary':'info'" class="cp" @click="signalFilter=f.k">{{ f.l }}</ElTag></div> <ElBadge :value="visibleSignals.length" :max="99" style="margin-left:4px" /></div>
        <div class="sl">
          ...
          <div v-for="sig in visibleSignals" :key="sig.ts_code + sig.strategy" class="sig-row" :title="`${sig.ts_code} ${sig.stock_name}\n策略: ${sig.strategy_name}\n量比: ${sig.volume_ratio?.toFixed(1) || '-'}\n换手: ${sig.turnover_rate?.toFixed(1) || '-'}%\n${sig.reason}`">
            <ElTag size="small" :color="strategyMeta[sig.strategy]?.color || 'var(--text-tertiary)'" class="tag-solid" style="min-width:52px;text-align:center">{{ displayStrategyName(sig.strategy, sig.strategy_name) }}</ElTag><ElTag v-if="sig.signal_status === 'executed'" size="small" type="success">已买</ElTag><ElTag v-if="sig.signal_status === 'skipped'" size="small" type="warning">跳过</ElTag><ElTag v-if="sig.signal_status === 'expired'" size="small" type="info">过期</ElTag><span class="code">{{ sig.ts_code }}</span><span class="name">{{ sig.stock_name }}</span><span v-if="sig.signal_status === 'new' && sigRemaining(sig) >= 0" class="expire-tag" :class="{ urgent: sigRemaining(sig) < 60000 }">⏱{{ formatRemaining(sigRemaining(sig)) }}</span><span :class="(sig.pct_chg || 0) >= 0 ? 'up' : 'down'" class="pct ml-auto" style="font-weight:600">{{ (sig.pct_chg || 0) >= 0 ? '+' : '' }}{{ (sig.pct_chg || 0).toFixed(1) }}%</span><ElButton v-if="!dryRun && sig.signal_status === 'new' && !sig._debug_signal && !sig._historical_signal" size="small" type="danger" plain @click="quickBuy(sig)" class="btn-xs">买</ElButton><ElButton v-if="sig.decision_detail" size="small" type="info" plain @click="openTradeDetail(sig.ts_code)" class="btn-xs">🔍</ElButton><ElButton v-if="sig.layer_trace" size="small" type="warning" plain @click="openScanTrace(sig.ts_code)" class="btn-xs">🧪</ElButton>
          </div>
        </div>
      </div>
    </div>
