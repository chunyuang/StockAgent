...
async function fetchScannerFast() { try { const r = await api.get(`${scannerApi}/all`); if (r?.success) { const d = r.data; if (d.status) status.value = d.status; if (d.signals) signals.value = d.signals; if (d.positions) positions.value = d.positions; if (d.timeline) timeline.value = d.timeline; if (d.orders) orders.value = d.orders } } catch (e) { console.error(e) } }
