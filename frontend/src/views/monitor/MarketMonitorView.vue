      ...
      <div class="mm-right">
        ...
        <div class="sl">
          ...
          <div v-for="pos in [...positions].sort((a, b) => a.profit_pct - b.profit_pct)" :key="pos.ts_code" class="pos-card">
            <div class="pos-top"><span class="code">{{ pos.ts_code }}</span><span class="name">{{ pos.stock_name }}</span><ElTag size="small" :color="strategyMeta[pos.strategy]?.color || '#909399'" style="color:#fff;border:none;font-size:10px">{{ pos.strategy_name || strategyCN(pos.strategy) }}</ElTag><span :class="pos.profit_pct >= 0 ? 'up' : 'down'" class="pct">{{ pos.profit_pct >= 0 ? '+' : '' }}{{ pos.profit_pct.toFixed(1) }}%</span></div>
...
.sig-row { display: flex; align-items: center; gap: 5px; padding: 4px 8px; margin-bottom: 2px; background: #fff; border-radius: 4px; border: 1px solid #ebeef5; font-size: 12px; }
...
.dr-g { display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; margin-bottom: 4px; }
.dr-i { display: flex; flex-direction: column; gap: 1px; }
