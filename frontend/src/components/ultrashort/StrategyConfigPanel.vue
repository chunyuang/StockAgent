...
const auctionFilterTitle = computed(() => `⏰ 竞价过滤 (涨幅${(props.form.auctionFilter.min_auction_pct * 100).toFixed(1)}%~${(props.form.auctionFilter.max_auction_pct * 100).toFixed(1)}%, 成交额≥${props.form.auctionFilter.min_auction_amount}万, 量比≥${props.form.auctionFilter.min_auction_volume_ratio})`)
...
const limitUpOpenTitle = computed(() => `📈 涨停开板策略 (连板≥${props.form.strategyConfigs.limit_up_open.params.min_consecutive_limit}板, 开板≤${props.form.strategyConfigs.limit_up_open.params.max_open_duration}分钟, 回封封单≥${props.form.strategyConfigs.limit_up_open.params.min_seal_after_open ?? props.form.strategyConfigs.limit_up_open.params.min_seal_orders ?? '?'}万元, 换手≥${props.form.strategyConfigs.limit_up_open.params.min_turnover_rate}%, 止损${(props.form.strategyConfigs.limit_up_open.riskParams.stop_loss_pct * 100).toFixed(1)}%/止盈${(props.form.strategyConfigs.limit_up_open.riskParams.take_profit_pct * 100).toFixed(1)}%, 持仓${props.form.strategyConfigs.limit_up_open.riskParams.max_hold_days}天, 滑点${(props.form.strategyConfigs.limit_up_open.riskParams.slippage_pct * 1000).toFixed(1)}‰)`)
  ...
  <ElCard class="config-card">
    <template #header>
      ...
      <ElCollapseItem name="auctionFilter">
        <template #title><span class="strategy-toggle" :class="{ on: form.auctionFilter.enabled }" @click.stop="form.auctionFilter.enabled = !form.auctionFilter.enabled">{{ form.auctionFilter.enabled ? "✅" : "❌" }}</span> {{ auctionFilterTitle }}, 未匹配量正: <span class="strategy-toggle" :class="{ on: form.auctionFilter.min_unmatched_volume_positive }" @click.stop="form.auctionFilter.min_unmatched_volume_positive = !form.auctionFilter.min_unmatched_volume_positive">{{ form.auctionFilter.min_unmatched_volume_positive ? "✅" : "❌" }}</span></template>
        <div class="item-layout"><div class="item-left">
        <ElForm label-width="160px">
          ...
          <div class="flow-strategies">
            ...
            <div v-if="form.strategyConfigs.limit_up_open.enabled" class="flow-strategy-card">
              ...
              <div class="flow-logic">
                ...
                <div class="flow-cond">③ 回封后封单 ≥ {{ form.strategyConfigs.limit_up_open.params.min_seal_after_open ?? form.strategyConfigs.limit_up_open.params.min_seal_orders ?? '?' }}万元?</div>
