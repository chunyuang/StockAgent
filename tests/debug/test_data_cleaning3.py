...
async def test_data_cleaning():
    ...
    # 不使用任何排除规则，只看停牌是否被剔除
    tradable_stocks = await universe_mgr.get_universe(UniverseType.ALL_A, test_date, exclude_rules=[])
    ...
    universe_with_st = await universe_mgr.get_universe(UniverseType.ALL_A, test_date, exclude_rules=[])
    universe_without_st = await universe_mgr.get_universe(UniverseType.ALL_A, test_date, exclude_rules=[ExcludeRule.ST])
    ...
    # 2. 测试次新股剔除是否生效
    universe_with_new = await universe_mgr.get_universe(UniverseType.ALL_A, test_date, exclude_rules=[])
    universe_without_new = await universe_mgr.get_universe(UniverseType.ALL_A, test_date, exclude_rules=[ExcludeRule.NEW_STOCK])
    ...
    universe_all = await universe_mgr.get_universe(UniverseType.ALL_A, test_date, exclude_rules=[])
    universe_clean = await universe_mgr.get_universe(UniverseType.ALL_A, test_date, exclude_rules=[ExcludeRule.ST, ExcludeRule.NEW_STOCK])
