...
class BacktestWizard:
    ...
    def confirm_date_range(self, default_start: str = "20251215", default_end: str = "20260318") -> Tuple[str, str]:
        ...
        print(f"  格式: YYYYMMDD YYYYMMDD，例如: 20251215 20260318")
        print()
    ...
    def check_data_integrity(self) -> Tuple[bool, List[str]]:
        ...
        if self.strategy_type == "A":
            # 五大超短只需要 stock_daily
            required = [
                ("stock_daily", ["trade_date", "ts_code", "open", "high", "low", "close",
                             "pre_close", "vol", "amount", "up_limit", "down_limit"],
             "stock_daily - 日线行情（含涨跌停价）")
            ]
        elif self.strategy_type == "B":
            # 因子选股需要 stock_daily + daily_basic + fina_indicator
            required = [
                ("stock_daily", ["trade_date", "ts_code", "open", "high", "low", "close",
                             "pre_close", "vol", "amount", "up_limit", "down_limit"],
             "stock_daily - 日线行情"),
        ...
        for collection_name, required_fields, description in required:
            ...
            # 数据库中 trade_date 存储为 int 类型，需要转换
            # 如果是 AKShare 数据源，还需要按 source 过滤
            start_int = int(self.start_date)
            end_int = int(self.end_date)
            query = {
                "trade_date": {"$gte": start_int, "$lte": end_int}
            }
            # 如果数据源是 AKShare 或 Tushare（已标记source），按source过滤
            if self.data_source == 2:  # AKShare
                query["source"] = "ak"
            elif self.data_source == 1 or self.data_source == 3:  # Tushare / 本地MongoDB
                # 如果有source标记，只查询ts数据
                pass  # 不添加过滤，保持兼容
            ...
            # 检查抽样一条看字段是否存在
            sample = self.db[collection_name].find_one(query)
        ...
        # 检查涨跌停价补全 (只对 stock_daily 需要)
        if "stock_daily" in [x[0] for x in required]:
            print(f"  检查涨跌停价完整性 ... ", end="", flush=True)
            # 数据库中 trade_date 存储为 int 类型，需要转换
            start_int = int(self.start_date)
            end_int = int(self.end_date)
            query = {
            ...
            # 如果是 AKShare 数据源，按 source 过滤
            if self.data_source == 2:
                query["source"] = "ak"
            count_missing = self.db['stock_daily'].count_documents(query)
            if count_missing > 0:
                print(f"⚠️  {count_missing} 条记录缺少 up_limit/down_limit")
                print(f"    需要运行: python backtest_module/data_download/add_up_down_limit.py")
                missing.append(f"stock_daily: {count_missing} 条记录缺少涨跌停价")
                all_ok = False
            else:
                print("✓ 全部完整")
        ...
        if all_ok:
            ...
            if "limit_list" in [m.split(":")[0] for m in missing]:
                print(f"  python backtest_module/data_download/tushare_limit_list.py {self.start_date} {self.end_date}")
            if "daily_basic" in [m.split(":")[0] for m in missing]:
                print(f"  python backtest_module/data_download/tushare_daily_basic_by_stock.py {self.start_date} {self.end_date}")
            if "lhb" in [m.split(":")[0] for m in missing]:
                print(f"  python backtest_module/data_download/tushare_lhb.py {self.start_date} {self.end_date}")
            if "stock_daily" in [m.split(":")[0] for m in missing] and "涨跌停" in "".join(missing):
                print(f"  python backtest_module/data_download/add_up_down_limit.py")
            print()
    ...
    def summary_config(self):
        ...
        print(f"  时间戳: 回测完成日期 YYYYMMDD (便于区分不同回测)")
        print()
        print("="*60)
        print()
    ...
    def run_backtest(self):
        ...
        # 导入并运行回测脚本
        # 根据数据源选择不同的脚本
        # 当前工作目录是 backtest_module/，所以路径要相对于这里
        if self.strategy_type == "A":
            # 五大超短策略 - 根据数据源选择脚本
            if self.data_source == 2:
                # AKShare 纯免费数据源，使用独立脚本
                script = "backtest_engine/scripts/run_strategies_backtest_3months_akshare_only.py"
            else:
                # Tushare / 本地 MongoDB，使用原有脚本
                script = "backtest_engine/scripts/run_strategies_backtest_3months.py"
        elif self.strategy_type == "B":
            script = "backtest_engine/scripts/run_factor_backtest_monthly.py"
        else:  # C
            script = "backtest_engine/scripts/run_emotion_cycle_backtest.py"
        ...
        g['INITIAL_CAPITAL'] = self.initial_capital
        g['LIQUIDITY_THRESHOLD'] = self.liquidity_threshold
        g['MAX_POSITION_PERCENT'] = 20  # 默认单票 20%
    ...
    def save_current_config(self):
        ...
        print(f"  下次启动: python backtest_module/run_backtest_wizard.py --config {config_name}")
