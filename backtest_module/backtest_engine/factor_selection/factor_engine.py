...
class FactorEngine:
    ...
    async def compute_factors(
        ...
        Args:
            ...
            liquidity_threshold: 如果设置，会额外加载今日 amount 字段用于流动性过滤
        ...
        # 如果需要流动性过滤，额外添加今日的 amount 字段
        if liquidity_threshold is not None and "daily" in data:
            # 提取每个股票今日的 amount
            today_amount = {}
            trade_date_int = int(trade_date)
            for ts_code, df in data["daily"].items():
                if trade_date_int in df.index:
                    today_amount[ts_code] = df.loc[trade_date_int, "amount"]
                else:
                    today_amount[ts_code] = np.nan
            result["amount"] = result["ts_code"].map(today_amount)
    ...
    async def _load_all_data(
        ...
        if "daily" in data_sources:
            data["daily"] = await self._load_daily_data(stocks, start_date, end_date, self.source)
        
        # 加载 daily_basic 数据
        if "daily_basic" in data_sources:
            data["daily_basic"] = await self._load_daily_basic_data(stocks, start_date, end_date, self.source)
    ...
    async def _load_daily_data(
        ...
        """加载日线数据
        Args:
            source: 数据源过滤 ("ak" for AKShare, "ts" for Tushare, None for no filter)
        """
    ...
    def select_top_stocks(
        ...
        """
        选出得分最高的 N 只股票
        过滤掉流动性不足的股票 (amount < liquidity_threshold)
        liquidity_threshold 单位：万元，因为 AKShare 存储 amount 单位就是万元
        """
        ...
        # 流动性过滤：amount < threshold 过滤掉
        if liquidity_threshold is not None:
            # amount 来自最后一个交易日，单位万元
            valid_df = valid_df[valid_df['amount'] >= liquidity_threshold]
