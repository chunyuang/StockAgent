...
> 版本: v2.9.75 | 日期: 2026-06-03 | 基线分支: audit/V75-backtest-review
...
| v2.9.74 MarketMonitorView TS6133清理(44个未使用解构变量消除,TS错误17→1)+OpsTab提取(provide/inject模式,86行)+全局TS错误7→1+stockApi补全+前端build零错误+10新增测试+1574 scanner+51 backtest全通过 |
| v2.9.73 Scanner访问器接口9方法+scanner_utils/position_manager getattr/hasattr消除+类属性默认值补齐+30新增测试+1620全通过 |
...
| v2.9.60 risk_watchdog 5方法提取(_run_checks 55→19+_check_position_health 57→27+_find_overdue_positions+_try_auto_recovery+_update_overall_status)+signal_manager 3方法提取(_execute_single_buy 55→26+_calc_buy_shares+_check_buy_quality+_apply_buy_slippage)+9新增测试+1459全通过 |
...
| v2.9.70 对齐统计持久化+通知+ReviewTab+TS修复✅ | v2.9.71 Health拆分(206→40L)+卖出trace_id+WS状态指示器+TS零错误✅
...
## 五十四、v2.9.73 Scanner访问器接口9方法 + scanner_utils/position_manager getattr/hasattr消除 (2026-06-03)

### 54.1 设计目标

1. **🔴 Scanner访问器接口**: 9个公开访问器方法替代scanner_utils/position_manager对scanner内部状态的getattr/hasattr穿透访问
2. **🟡 scanner_utils.py getattr/hasattr清理**: 10处getattr/hasattr(scanner,...)→访问器调用, 10→0
3. **🟡 position_manager.py hasattr清理**: 1处hasattr(scanner, '_trade_date')→scanner.get_trade_date()
4. **🟢 类属性默认值补齐**: _event_bus/_circuit_breaker/_current_sentiment/_current_position_ratio 4个属性提升为类属性, 消除__new__实例的AttributeError

### 54.2 访问器方法

| 访问器 | 返回类型 | 对应内部属性 |
|---|---|---|
| `get_current_sentiment()` | Dict | `_current_sentiment` |
| `get_current_position_ratio()` | Optional[float] | `_current_position_ratio` |
| `get_scan_error_count()` | int | `_scan_loop_error_count` |
| `get_event_bus()` | Optional[ScannerEventBus] | `_event_bus` |
| `get_risk_thread()` | Optional[threading.Thread] | `_risk_thread` |
| `get_circuit_breaker()` | Dict | `_circuit_breaker` |
| `get_risk_thread_restarts()` | int | `_risk_thread_restarts` |
| `is_risk_running()` | bool | `_risk_running` |
| `get_trade_date()` | str | `_trade_date` |

### 54.3 变更文件

| 文件 | 变更 |
|---|---|
| scanner.py | 9个访问器方法+4个类属性默认值补齐 |
| scanner_utils.py | 10处getattr/hasattr→访问器调用 |
| position_manager.py | 1处hasattr→访问器调用 |
| web/api/scanner_system.py | _DESIGN_DOC_VERSION→v2.9.73 |
| test_v2973_accessor_getattr_cleanup.py | 新增30测试 |
| 3个测试文件(mock适配) | test_health_emotion_pending+test_sell_signal_checker+test_v295_stability |
| 23个版本断言文件 | v2.9.72→v2.9.73 |

### 54.4 getattr/hasattr消除统计

| 文件 | v2.9.72 | v2.9.73 | 减少 |
|---|---|---|---|
| scanner_utils.py | 10 | 0 | -10 |
| position_manager.py | 2 | 1 | -1 |
| **合计(scanner状态穿透)** | **12** | **1** | **-11** |

> 注: position_manager剩余1处getattr在成本价提取(_extract_cost_price),属于Position对象多态访问,非scanner穿透。

### 54.5 测试覆盖 (30新增)

| 测试类 | 用例数 | 覆盖点 |
|---|---|---|
| TestScannerAccessorMethods | 10 | 9个访问器存在+返回类型+区域注释 |
| TestScannerUtilsGetattrCleanup | 15 | 8处getattr/hasattr消除验证+7处访问器使用验证+剩余数统计 |
| TestPositionManagerGetattrCleanup | 2 | hasattr消除+访问器使用 |
| TestVersionV2973 | 2 | 版本常量+断言文件 |
| TestNoBacktestRegression | 1 | 零回测影响 |

**全量测试**: 1569 scanner + 51 backtest = 1620 passed (0 failed)

### 54.6 回测影响

零。所有变更仅影响market_monitor模块内部接口优化, 回测引擎零文件修改。
...
## 五十一、v2.9.62 6模块方法提取 + quote_manager降级拆分 + tiered_scanner数据映射提取 + strategy_scorer异动检测拆分 (2026-06-02)

### 51.1 设计目标

1. **🟡 quote_manager._fetch_eastmoney_data拆分**: 54行→22行(-59%), 提取3个子方法(_map_em_item_to_realtime/@staticmethod+_handle_em_degrade_recovery+_handle_em_fetch_failure)
2. **🟡 tiered_scanner._fetch_l3_prices拆分**: 52行→26行(-50%), 提取4个子方法(_fetch_l3_eastmoney_batch+_fetch_l3_biying_snapshot+_map_quote_to_price_dict/@staticmethod+_map_l2_quote_to_dict/@staticmethod)
3. **🟡 signal_manager._post_buy_success拆分**: 52行→8行(-85%), 提取2个子方法(_build_buy_timeline_entry+_emit_buy_events)
4. **🟡 position_manager.update_trailing_stops拆分**: 52行→31行(-40%), 提取_update_single_trailing_stop
5. **🟡 position_manager.retry_pending_sells拆分**: 52行→21行(-60%), 提取_retry_single_pending_sell
6. **🟡 live_filter_pipeline._auction_filter拆分**: 52行→32行(-38%), 提取2个@staticmethod(_calc_opening_pct+_check_auction_pass)
7. **🟡 strategy_scorer._check_single_anomaly拆分**: 52行→37行(-29%), 提取3个子方法(_check_broken_board+_check_strong_limit+_check_surge)

### 51.2 变更文件

| 文件 | 变更 |
|---|---|
| quote_manager.py | 3个方法提取(_map_em_item_to_realtime+_handle_em_degrade_recovery+_handle_em_fetch_failure) |
| tiered_scanner.py | 4个方法提取(_fetch_l3_eastmoney_batch+_fetch_l3_biying_snapshot+_map_quote_to_price_dict+_map_l2_quote_to_dict) |
| signal_manager.py | 2个方法提取(_build_buy_timeline_entry+_emit_buy_events) |
| position_manager.py | 2个方法提取(_update_single_trailing_stop+_retry_single_pending_sell) |
| live_filter_pipeline.py | 2个@staticmethod提取(_calc_opening_pct+_check_auction_pass) |
| strategy_scorer.py | 3个方法提取(_check_broken_board+_check_strong_limit+_check_surge) |
| web/api/scanner.py | _DESIGN_DOC_VERSION→v2.9.62 |
| test_v2962_method_extractions.py | 新增48测试 |
| 15个版本断言文件 | v2.9.61→v2.9.62 |

### 51.3 方法行数改善

| 方法 | 之前 | 之后 | 变化 |
|---|---|---|---|
| _fetch_eastmoney_data | 54行 | 22行 | -59% |
| _fetch_l3_prices | 52行 | 26行 | -50% |
| _post_buy_success | 52行 | 8行 | -85% |
| update_trailing_stops | 52行 | 31行 | -40% |
| retry_pending_sells | 52行 | 21行 | -60% |
| _auction_filter | 52行 | 32行 | -38% |
| _check_single_anomaly | 52行 | 37行 | -29% |
| **新增** | | | |
| _map_em_item_to_realtime | - | 17行 | @staticmethod 数据映射 |
| _handle_em_degrade_recovery | - | 10行 | 降级恢复 |
| _handle_em_fetch_failure | - | 12行 | 失败处理 |
| _fetch_l3_eastmoney_batch | - | 12行 | 东财批量 |
| _fetch_l3_biying_snapshot | - | 18行 | 必盈快照 |
| _map_quote_to_price_dict | - | 8行 | @staticmethod quote→价格 |
| _map_l2_quote_to_dict | - | 8行 | @staticmethod L2映射 |
| _build_buy_timeline_entry | - | 28行 | 时间线构建 |
| _emit_buy_events | - | 17行 | 事件推送 |
| _update_single_trailing_stop | - | 28行 | 单票追踪止损 |
| _retry_single_pending_sell | - | 30行 | 单票重试 |
| _calc_opening_pct | - | 11行 | @staticmethod 竞价涨幅 |
| _check_auction_pass | - | 12行 | @staticmethod 竞价过滤 |
| _check_broken_board | - | 13行 | 炸板检测 |
| _check_strong_limit | - | 13行 | 强势涨停 |
| _check_surge | - | 14行 | 急速拉升 |

### 51.4 超过50行方法数

- v2.9.61: 12个方法超过50行
- v2.9.62: 5个方法超过50行(-7, -58%)
- 剩余5个均为编排方法(scan_once 55L, _apply_filter_layers 55L, _l3_scan 54L, _merge_limit_pool_data 52L, _l2_scan 52L), 结构清晰无需进一步拆分

### 51.5 测试覆盖 (48新增)

| 测试类 | 用例数 | 覆盖点 |
|---|---|---|
| TestQuoteManagerV2962Extraction | 8 | 3个提取方法+行数+@staticmethod+使用验证 |
| TestTieredScannerV2962Extraction | 7 | 4个提取方法+行数+使用验证 |
| TestSignalManagerV2962Extraction | 6 | 2个提取方法+行数+使用验证 |
| TestPositionManagerV2962Extraction | 8 | 2个提取方法+行数+使用验证 |
| TestLiveFilterPipelineV2962Extraction | 8 | 2个@staticmethod+行数+使用验证 |
| TestStrategyScorerV2962Extraction | 8 | 3个提取方法+行数+使用验证 |
| TestBigMethodsReductionV2962 | 1 | >50行方法数减少 |
| TestNoBacktestRegressionV2962 | 2 | 回测零影响+版本常量 |

**全量测试**: 1469 passed (0 failed)
...
## 五十、v2.9.59 emotion_cycle 5方法提取 + signal_dispatcher 3方法提取 + event_bus 1方法提取 + strategy_scorer 3方法提取 + runtime_persistence 3方法提取 + execution_quality 2方法提取 + risk_watchdog 2方法提取 (2026-06-02)

### 50.1 设计目标

1. **🟡 emotion_cycle._try_add_pending_sell提取**: 跌停挂起卖出逻辑去重(2处→1处), build_emotion_sell_list 64→47行
2. **🟡 emotion_cycle.update_sentiment_score拆分**: 64→11行(-83%), 提取4个子方法(_fetch_limit_stats/_fetch_up_down_ratio/_calc_sentiment_score/_persist_sentiment_score)
3. **🟡 signal_dispatcher.dispatch拆分**: 63→15行(-76%), 提取3个子方法(_should_dedup/_dispatch_to_channels/_record_dispatch)
4. **🟡 scanner_event_bus.emit拆分**: 59→21行(-64%), 提取1个子方法(_invoke_handler)
5. **🟡 strategy_scorer.merge_factors拆分**: 58→7行(-88%), 提取3个子方法(_realtime_to_dataframe/_classify_limit/_merge_with_daily_factors)
6. **🟡 runtime_persistence.post_sell_cleanup拆分**: 57→22行(-61%), 提取3个子方法(_classify_sell_stats/_emit_sell_events/_persist_sell_state)
7. **🟡 execution_quality.check_buy拆分**: 71→19行(-73%), 提取2个子方法(_check_buy_basics/_check_buy_position_and_cash)
8. **🟡 risk_watchdog._check_drawdown拆分**: 69→28行(-59%), 提取2个子方法(_compute_drawdowns/_judge_drawdown_status)

### 50.2 变更文件

| 文件 | 变更 |
|---|---|
| emotion_cycle.py | _try_add_pending_sell去重+update_sentiment_score 4子方法+build_emotion_sell_list简化 |
| signal_dispatcher.py | dispatch 3子方法(63→15行) |
| scanner_event_bus.py | emit 1子方法(59→21行) |
| strategy_scorer.py | merge_factors 3子方法(58→7行) |
| runtime_persistence.py | post_sell_cleanup 3子方法(57→22行) |
| execution_quality.py | check_buy 2子方法(71→19行) |
| risk_watchdog.py | _check_drawdown 2子方法(69→28行) + Tuple导入 |
| web/api/scanner.py | _DESIGN_DOC_VERSION→v2.9.59 |
| test_v2958_method_extractions.py | 新增21测试 |
| 13个版本断言文件 | v2.9.58→v2.9.59 |
| 3个旧测试文件 | 适配提取后的源码检查 |
...
| 方法 | 之前 | 之后 | 变化 |
|---|---|---|---|
| update_sentiment_score | 64行 | 11行 | -83% |
| dispatch | 63行 | 15行 | -76% |
| merge_factors | 58行 | 7行 | -88% |
| post_sell_cleanup | 57行 | 22行 | -61% |
| check_buy | 71行 | 19行 | -73% |
| _check_drawdown | 69行 | 28行 | -59% |
| emit | 59行 | 21行 | -64% |
| build_emotion_sell_list | 64行 | 47行 | -27% |

### 50.4 超过50行方法数

- v2.9.58: 27个方法超过50行
- v2.9.59: 21个方法超过50行(-6, -22%)

### 50.5 测试覆盖 (21新增)

| 测试类 | 用例数 | 覆盖点 |
|---|---|---|
| TestEmotionCycleV2959Extraction | 7 | _try_add_pending_sell+4子方法+行数 |
| TestSignalDispatcherExtraction | 4 | 3个提取方法+行数 |
| TestEventBusExtraction | 2 | emit行数+_invoke_handler |
| TestStrategyScorerMergeFactorsExtraction | 4 | 3个提取方法+行数 |
| TestRuntimePersistencePostSellExtraction | 4 | 3个提取方法+行数 |

**全量测试**: 1450 passed (0 failed)
...
## 四十九、v2.9.58 replay_provider拆分 + strategy_scorer提取 + broker拆分 + emotion_cycle提取 + strategy_param_center提取 (2026-06-02)

### 49.1 设计目标

1. **🟡 replay_provider.get_replay_data拆分**: 129行→41行(-68%), 提取4个子方法(_load_daily_data/_enrich_daily_basic/_load_limit_pools/_compute_volume_ratios)
2. **🟡 strategy_scorer.apply_strategies提取**: 96行→31行(-68%), 提取_apply_filter_conditions和_build_signal_from_row
3. **🟡 strategy_scorer.detect_anomalies提取**: 81行→18行(-78%), 提取_check_single_anomaly
4. **🟡 broker.place_order拆分**: 80行→36行(-55%), 提取_execute_order_fill
5. **🟡 emotion_cycle.calculate_daily_emotion提取**: 86行→28行(-67%), 提取_collect_emotion_factors和_build_emotion_score
6. **🟡 strategy_param_center.update_strategy_params提取**: 83行→41行(-51%), 提取_persist_param_update/_record_param_history/_notify_param_update

### 49.2 replay_provider拆分

**问题**: `get_replay_data` 129行, 包含4个独立阶段(日线加载/PE-PB补充/涨跌停池/量比计算)混杂在一起, 且有死代码(vol_cache从未使用)和未使用导入(from datetime import timedelta)。

**修复**: 按职责拆分为4个子方法:

| 方法 | 职责 | 行数 |
|---|---|---|
| `get_replay_data` | 编排4步+缓存 | 41 |
| `_load_daily_data` | 从stock_daily_ak_full加载日线 | 22 |
| `_enrich_daily_basic` | 从daily_basic补充PE/PB | 12 |
| `_load_limit_pools` | 涨停池/跌停池 | 30 |
| `_compute_volume_ratios` | 5日均量→量比 | 27 |

**额外修复**: 消除死代码vol_cache和未使用的from datetime import timedelta导入。

### 49.3 strategy_scorer提取

**问题**: `apply_strategies` 96行, 筛选条件应用和信号构建混杂; `detect_anomalies` 81行, 3种异动类型检查与循环混杂。

**修复**:

| 方法 | 职责 | 行数 |
|---|---|---|
| `apply_strategies` | 编排(策略循环+去重) | 31 |
| `_apply_filter_conditions` | 将回测筛选条件→bool mask | 18 |
| `_build_signal_from_row` | DataFrame行→ScanSignal | 38 |
| `detect_anomalies` | 编排(遍历+跳过) | 18 |
| `_check_single_anomaly` | 单只股票异动检测 | 42 |

**额外修复**: Optional未导入导致NameError(仅在_check_single_anomaly的返回类型注解中使用)。

### 49.4 broker.place_order拆分

**问题**: `place_order` 80行, 前置检查/买入卖出校验与撮合执行/持仓更新/持久化混杂在一起。

**修复**:

| 方法 | 职责 | 行数 |
|---|---|---|
| `place_order` | 编排(检查→校验→撮合) | 36 |
| `_execute_order_fill` | 撮合+执行+持久化 | 34 |

### 49.5 emotion_cycle提取

**问题**: `calculate_daily_emotion` 86行, 数据收集(5个因子)和结果构建混在一起。

**修复**:

| 方法 | 职责 | 类型 | 行数 |
|---|---|---|---|
| `calculate_daily_emotion` | 编排(缓存→收集→打分→构建) | async | 28 |
| `_collect_emotion_factors` | 收集5个因子(涨跌停/连板/涨跌比/溢价) | async | 30 |
| `_build_emotion_score` | 构建EmotionScore结果对象 | sync | 18 |

### 49.6 strategy_param_center提取

**问题**: `update_strategy_params` 83行, 合并/持久化/历史记录/通知/审计混在一起。

**修复**:

| 方法 | 职责 | 行数 |
|---|---|---|
| `update_strategy_params` | 编排(合并→持久化→历史→通知→审计) | 41 |
| `_persist_param_update` | MongoDB upsert | 7 |
| `_record_param_history` | 插入历史文档 | 12 |
| `_notify_param_update` | 通知Scanner热更新 | 8 |

### 49.7 变更文件

| 文件 | 变更 |
|---|---|
| replay_provider.py | get_replay_data拆分4个子方法(129→41行) + 死代码消除 |
| strategy_scorer.py | apply_strategies拆分2个子方法(96→31) + detect_anomalies拆分1个子方法(81→18) + Optional导入 |
| broker.py | place_order拆分(80→36行) |
| emotion_cycle.py | calculate_daily_emotion拆分2个子方法(86→28行) |
| strategy_param_center.py | update_strategy_params拆分3个子方法(83→41行) |
| web/api/scanner.py | _DESIGN_DOC_VERSION→v2.9.58 |
| test_v2958_method_extractions.py | 新增26测试 |
| 14个版本断言文件 | v2.9.57→v2.9.58 |

### 49.8 方法行数改善

| 方法 | v2.9.57 | v2.9.58 | 变化 |
|---|---|---|---|
| get_replay_data | 129行 | 41行 | -68% |
| apply_strategies | 96行 | 31行 | -68% |
| detect_anomalies | 81行 | 18行 | -78% |
| place_order | 80行 | 36行 | -55% |
| calculate_daily_emotion | 86行 | 28行 | -67% |
| update_strategy_params | 83行 | 41行 | -51% |
| **新增** | | | |
| _load_daily_data | - | 22行 | 日线加载 |
| _enrich_daily_basic | - | 12行 | PE/PB补充 |
| _load_limit_pools | - | 30行 | 涨跌停池 |
| _compute_volume_ratios | - | 27行 | 量比计算 |
| _apply_filter_conditions | - | 18行 | 筛选条件→mask |
| _build_signal_from_row | - | 38行 | 信号构建 |
| _check_single_anomaly | - | 42行 | 单票异动检测 |
| _execute_order_fill | - | 34行 | 撮合+执行 |
| _collect_emotion_factors | - | 30行 | 因子收集 |
| _build_emotion_score | - | 18行 | 结果构建 |
| _persist_param_update | - | 7行 | MongoDB upsert |
| _record_param_history | - | 12行 | 历史记录 |
| _notify_param_update | - | 8行 | 热更新通知 |

### 49.9 超过50行方法数

- 提取前(v2.9.57): 34个方法超过50行
- 提取后(v2.9.58): 29个方法超过50行(-5, -15%)

### 49.10 测试覆盖 (26新增)

| 测试类 | 用例数 | 覆盖点 |
|---|---|---|
| TestReplayProviderDecomposition | 4 | 4个子方法存在+行数+死代码消除 |
| TestStrategyScorerExtraction | 6 | 3个提取方法存在+行数+Optional导入 |
| TestBrokerPlaceOrderExtraction | 3 | 提取方法存在+行数 |
| TestEmotionCycleExtraction | 5 | 2个提取方法存在+行数+async/sync类型 |
| TestStrategyParamCenterExtraction | 4 | 3个提取方法存在+行数 |
| TestBigMethodsReduction | 1 | >50行方法数减少 |
| TestNoBacktestRegressionV2958 | 3 | 回测零影响+版本常量 |

**全量测试**: 1420 passed (0 failed)
...
| # | 问题 | 影响 | 根因 |
|---|---|---|---|
| P0-1 | God Class 2907行 | ~~改一处坏全局~~ | v2.9架构解耦: 2907→1406行(-52%) |
| P0-2 | 盘中崩溃状态丢失 | ~~重启后风控失效~~ | v2.9.1持9久化+自动恢复 |
| P0-3 | 扫描和风控抢时间 | ~~止损延迟~~ | v2.9.3风控独立线程 |
| P0-4 | Redis Pub/Sub无保障 | ~~信号/状态丢失~~ | v2.9.7 List+ACK+Stream |
| P0-5 | 实盘-回测卖出逻辑分叉 | ~~回测结果不可信~~ | v2.9.6 compare双跑+灰度开关 |
| P0-6 | Scanner缺利润锁定 | ~~盘中利润回吐~~ | v2.9.64补齐 |
| P0-7 | 追踪止损/移动止损迁移后会丢 | 迁移后风控退化 | checker新增trailing_state参数 |
...
## 五十一、v2.9.61 live_filter_pipeline.apply提取 + emotion_cycle提取 + delegate_router路由表 + tiered_scanner提取 + broker._match提取 + risk_watchdog._check_heartbeat提取 (2026-06-02)

### 51.1 设计目标

1. **🟡 live_filter_pipeline.apply拆分**: 103行→48行(-53%), 提取2个子方法(_apply_filter_layers L2~L7编排+_finalize_traces最终标记)
2. **🟡 emotion_cycle.handle_emotion_phase_change拆分**: 64行→47行(-27%), 提取2个子方法(_execute_emotion_batch_sell分批卖出+_emit_rebalance_event事件推送)
3. **🟡 emotion_cycle._calculate_zt_premium拆分**: 54行→12行(-78%), 提取2个子方法(_get_prev_trade_date获取前交易日+_calc_avg_zt_premium批量计算溢价)
4. **🟡 scanner_delegate_router.resolve_delegate重构**: 62行→16行(-74%), if/elif链→_DISPATCH_TABLE路由表+6个独立_resolve_*函数
5. **🟡 tiered_scanner._l3_builtin_check拆分**: 58行→17行(-71%), 提取_check_single_position_stop_profit单票检查
6. **🟡 broker._match拆分**: 57行→28行(-51%), 提取_calc_dynamic_slippage动态滑点计算
7. **🟡 risk_watchdog._check_heartbeat拆分**: 51行→13行(-75%), 提取_judge_startup_heartbeat+_judge_heartbeat_elapsed

### 51.2 变更文件

| 文件 | 变更 |
|---|---|
| live_filter_pipeline.py | apply 103→48行+2方法提取(_apply_filter_layers 55行+_finalize_traces 13行) |
| emotion_cycle.py | handle_emotion_phase_change 64→47行+2方法提取+_calculate_zt_premium 54→12行+2方法提取 |
| scanner_delegate_router.py | resolve_delegate 62→16行(if/elif→_DISPATCH_TABLE)+6个_resolve_*函数+_resolve_module_delegate |
| tiered_scanner.py | _l3_builtin_check 58→17行+_check_single_position_stop_profit 32行提取 |
| broker.py | _match 57→28行+_calc_dynamic_slippage 32行提取 |
| risk_watchdog.py | _check_heartbeat 51→13行+_judge_startup_heartbeat+_judge_heartbeat_elapsed |
| web/api/scanner.py | _DESIGN_DOC_VERSION→v2.9.61 |
| test_v2961_method_extractions.py | 新增47测试 |
| test_v2948_pipeline_apply_extraction.py | 2测试适配(apply→_apply_filter_layers) |
| 7个版本断言文件 | v2.9.60→v2.9.61 |
...
|---|---|---|---|
| apply | 103行 | 48行 | -53% |
| handle_emotion_phase_change | 64行 | 47行 | -27% |
| resolve_delegate | 62行 | 16行 | -74% |
| _l3_builtin_check | 58行 | 17行 | -71% |
| _match | 57行 | 28行 | -51% |
| _check_heartbeat | 51行 | 13行 | -75% |
| _calculate_zt_premium | 54行 | 12行 | -78% |

### 51.4 超过50行方法数

- v2.9.60: 18个方法超过50行
- v2.9.61: 12个方法超过50行(-6, -33%)

### 51.5 测试覆盖 (47新增)

| 测试类 | 用例数 | 覆盖点 |
|---|---|---|
| TestLiveFilterPipelineExtraction | 9 | _apply_filter_layers+_finalize_traces+apply行数+委托 |
| TestEmotionCycleExtraction | 9 | 4个提取方法+行数+async |
| TestDelegateRouterDispatchTable | 10 | _DISPATCH_TABLE+6个_resolve_*+路由表使用+行数 |
| TestTieredScannerExtraction | 5 | _check_single_position_stop_profit+行数+委托 |
| TestBrokerMatchExtraction | 7 | _calc_dynamic_slippage+行数+规则保留 |
| TestRiskWatchdogHeartbeatExtraction | 5 | 2个提取方法+行数+HealthCheck返回 |
| TestBigMethodsReduction | 1 | >50行方法数减少 |
| TestNoBacktestRegressionV2961 | 2 | 回测零影响+版本常量 |

**全量测试**: 1421 passed (0 failed)
...
## 五十四、v2.9.65 P0-2 MarketMonitorView script拆分 + 测试断言适配 (2026-06-03)

### 54.1 设计目标

1. **P0-2 MarketMonitorView script拆分**: 将MarketMonitorView.vue的`<script setup>`从973行提取为`useScannerMonitor.ts` composable hook(1177行), MarketMonitorView.vue只保留template+style+composable调用(script约50行)
2. **测试断言适配**: `test_phase4_health_version.py`中`test_component_uses_store_for_freshness`断言从检查`useScannerStore`/`wsHook`更新为兼容`useScannerMonitor` composable模式
3. **版本常量**: v2.9.64→v2.9.65

### 54.2 设计决策

**composable vs Store**: 选择composable而非Pinia Store,因为useScannerMonitor是视图级逻辑(包含大量UI状态、ref、生命周期),不适合作为全局Store。Scanner Store仅用于跨组件共享的健康度/WS分发等数据。

**script不拆template/style**: template(1700+行)和style(900+行)保留在.vue文件中,避免拆分后标签匹配问题和样式隔离问题。后续可考虑模板组件化(按Tab拆分子组件)。

**composable拆分规划**: useScannerMonitor.ts 1177行仍是巨型composable,但此次仅做script外提,后续可按6个功能域进一步拆分:
- useScannerCore: 核心状态+WS+fetch
- useScannerPremarket: 盘前竞价Tab
- useScannerTrace: 扫描追踪Tab
- useScannerReview: 复盘Tab
- useScannerSentiment: 情绪Tab
- useScannerAutoTrade: 自动交易+参数对比

### 54.3 变更文件

| 文件 | 变更 |
|---|---|
| MarketMonitorView.vue | script 973行→50行(composable调用) |
| useScannerMonitor.ts | 新增1177行composable hook |
| test_phase4_health_version.py | 断言适配useScannerMonitor |
| test_v2965_composable_refactor.py | 新增8测试 |
| scanner_system.py | _DESIGN_DOC_VERSION→v2.9.65 |
| MARKET_MONITOR_OPTIMIZATION_DESIGN.md | v2.9.65修订记录 |

### 54.4 测试覆盖 (8新增)

| 测试类 | 用例数 | 覆盖点 |
|---|---|---|
| TestVersionV2965 | 1 | 版本常量v2.9.65 |
| TestMarketMonitorViewComposableRefactor | 5 | composable存在+view导入+script薄壳+Store+WS |
| TestPhase4TestFix | 1 | 测试断言适配composable模式 |
| TestNoBacktestRegression | 1 | 回测引擎核心文件未修改 |

**全量测试**: 1471 passed (1 failed→0 after fix)
...
## 五十八、v2.9.71 Health拆分 + 卖出trace_id + WS状态指示器 (2026-06-03)

### 58.1 变更摘要

1. **get_scanner_health拆分** — 206L→40L, 提取6个子方法(策略checks/数据源checks/风控指标/综合健康度/Scanner健康度/健康响应构建)
2. **卖出链路trace_id** — 每笔卖出生成唯一trace_id(如`risk-600036.SH-a1b2c3d4`), 贯穿timeline→EventBus→MongoDB, 端到端可追踪
3. **前端WS连接状态指示器** — 顶部状态栏新增WS连接状态(绿=WS/黄=重连/灰=离线), 含脉冲动画
4. **前端TS零错误** — 修复27个TS类型错误(api模块补齐方法签名、store返回类型注解、移除未使用导入)
5. **前端API模块补齐** — stockApi/userApi/authApi/taskApi补齐缺失方法, 从mock升级为真实API调用
...
| 文件 | 变更 |
|------|------|
| MarketMonitorView.vue | 顶部状态栏新增WS连接状态指示器(3色+脉冲动画) |
| useCoreMethods.ts | 暴露wsStatus/wsIsConnected/wsRetryCount |
| useScannerMonitor.ts | 透传WS状态到模板 |
| scanner.ts (store) | api.get/post返回类型注解Record<string, any> |
| scanner.ts (utils) | 移除未使用scannerApi常量 |
| stock.ts (api) | 补齐getMarketOverview/getRealtimeQuotes/getStockBasic/getIndustries |
| user.ts (api) | 补齐getCurrentUser/addToWatchlist/removeFromWatchlist |
| auth.ts (api) | 补齐loginWithCredentials/registerWithCredentials/changePassword |
| task.ts (api) | 补齐createTask/cancelTask/listTasks |
| client.ts | 移除未使用SCAN_TRACE_TIMEOUT |

### 58.4 方法行数统计

- `get_scanner_health`: 206L→40L ✅ (-80%)
- `_build_strategy_checks`: 28L (新提取)
- `_build_datasource_checks`: 20L (新提取)
- `_build_risk_metrics`: 49L (新提取)
- `_compute_overall_health`: 39L (新提取)
- `_build_scanner_health_dict`: 10L (新提取)
- `_build_health_response`: 27L (新提取)
- **零方法 > 50L** ✅

### 58.5 测试

- 21个版本断言文件更新(v2.9.70→v2.9.71)
- test_v2910_health_unification: state_lock检查适配拆分后的_build_risk_metrics
- test_v2958_method_extractions: post_sell_cleanup行数阈值25→35(新增trace_id参数)
...
## 六十、v2.9.74 MarketMonitorView TS6133清理 + 未使用解构变量消除 (2026-06-03)

### 60.1 设计目标

1. **🟡 TS6133错误清理**: MarketMonitorView.vue中26个未使用解构变量移除, vue-tsc错误从17→1(仅剩scanTraceCode的vue-tsc模板内插误报)
2. **🟢 前端build零错误**: vite build通过, 无新增编译错误
3. **🟢 版本常量**: v2.9.73→v2.9.74

### 60.2 未使用变量分析

MarketMonitorView.vue通过 `useScannerMonitor()` composable解构获取所有响应式变量和方法, 但其中26个仅在composable内部使用, 不需要在视图层解构:

| 变量 | 来源 | 未使用原因 |
|------|------|-----------|
| soundEnabled | core | composable内部自动使用 |
| limitPools, limitPoolTab | core | 已移至子组件 |
| healthData/Status/Emoji/CN/Class | core | 已移至SystemHealth子组件 |
| riskBarCollapsed | core | 移至RiskTab子组件 |
| globalRisk | core | composable内部使用 |
| nowMs | core | composable内部使用 |
| pnlHistory/perfData/perfChartOption | autoTrade | composable内部使用 |
| scanTraceDates/scanTraceList | scanTrace | composable内部使用 |
| reviewData/reviewHeroData/disciplineData/reviewForwardData | review | 已移至ReviewTab子组件 |
| brokers/dataSources | core | composable内部使用 |
| fetchAll/fetchScanner/fetchStrategies | core | composable内部自动调用 |
| playSignalSound | core | composable内部自动使用 |
| openLayerDebug | core | 模板中未直接调用 |

### 60.3 变更文件

| 文件 | 变更 |
|------|------|
| MarketMonitorView.vue | 26个未使用解构变量移除 + scanTraceCode注释标注 |
| scanner_system.py | _DESIGN_DOC_VERSION v2.9.73→v2.9.74 |
| test_v2974_ts6133_cleanup.py | 新增5测试 |
| 3个版本断言文件 | v2.9.73→v2.9.74 |
| 17个历史断言文件 | v2.9.73→v2.9.74 |

### 60.4 TS错误统计

| 指标 | v2.9.73 | v2.9.74 | 变化 |
|------|---------|---------|------|
| MarketMonitorView TS6133 | 17 | 1 | -16 |
| 全局vue-tsc错误 | 7 | 1 | -6 |
| MarketMonitorView行数 | 2472 | 2385 | -87(OpsTab提取) |
| 前端build | ✅ | ✅ | 无变化 |

> 注: 剩余1个TS6133(scanTraceCode)是vue-tsc误报——变量在模板内插中使用(`{{ scanTraceCode }}`), 但vue-tsc无法识别, 已加注释标注。

### 60.5 子组件提取

| 组件 | 行数 | 模式 | 说明 |
|------|------|------|------|
| OpsTab.vue | 120 | provide/inject | 运维Tab, 从MarketMonitorView提取86行模板 |
| scannerMonitorInject.ts | 20 | provide/inject | 注入key + useScannerMonitorInject()辅助函数 |

### 60.6 全局TS修复

| 文件 | 修复 |
|------|------|
| FactorReferencePanel.vue | 移除未使用Search导入 |
| StrategyConfigPanel.vue | 移除_activeDescription/currentSweepUnit + TS2367修复 |
| MainLayout.vue | logout重命名_logout |
| UltraShortBacktestViewV2.vue | reviewReport v-if类型安全 + 移除_configCollapsed |
| SettingsView.vue | 移除onMounted + 类型断言 |
| StockDetailView.vue | getStockDaily类型断言 |
| stock.ts | 补全getStockDaily API方法 |
...
| TestMarketMonitorViewTSCleanup | 3 | 44个变量移除+scanTraceCode标注+weeklyReviewData解构 |
...
## 61. v2.9.75: TypeScript错误全面修复 + Tab组件Props补齐

**日期**: 2026-06-03 | **提交**: 待定

### 61.1 变更摘要

修复v2.9.74 Tab提取遗留的16个TypeScript编译错误, 补齐子组件Props/Inject绑定。

### 61.2 修复清单

| 文件 | 修复内容 | TS错误消除 |
|------|----------|-----------|
| SentimentTab.vue | 提取intradayTradeMarkers/dailyTradeMarkers计算属性, 消除模板内隐式any回调 | 10× TS7006 |
| SentimentTab.vue | matrixTotal辅助函数替代模板内reduce | 1× TS6133 |
| PremarketTab.vue | Record<string,string>类型断言消除TS7053 | 1× TS7053 |
| PremarketTab.vue | executedCount/blockedCount计算属性替代模板内.filter()回调 | 2× TS7006 |
| HistoryTab.vue | 移除未使用的loading解构变量 | 1× TS6133 |
| MarketMonitorView.vue | scanTraceCode不再解构(子Tab通过inject获取) | 2× TS1005 |
| MarketMonitorView.vue | compareVisible/compareData加入ReviewTab props传递 | 2× TS2339 |
| ScanTraceTab.vue | 补齐8个模板使用但未解构的inject变量 | 6× TS2339/TS2551 |
| ReviewTab.vue | 新增compareVisible/compareData/dailyReportVisible/dailyReport/weeklyReportVisible props | 20× TS2339 |
| ReviewTab.vue | v-model改为:model-value+@update:model-value(props只读) | 3× 编译错误 |

### 61.3 最终TS错误状态

- vue-tsc: **1个已知false positive** (ScanTraceTab.vue scanTraceCode TS6133 — 变量在模板中使用但vue-tsc无法追踪inject模式)
- vite build: **0错误** ✅
...
| 测试 | 结果 |
|------|------|
| scanner tests | 1584 passed (含v2.9.75更新) |
| backtest tests | 51 passed ✅ |
| vue-tsc --noEmit | 1 false positive |
| vite build | 成功 ✅ |
