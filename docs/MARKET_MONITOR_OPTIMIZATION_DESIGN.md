...
| v2.9.74 MarketMonitorView TS6133清理(26个未使用解构变量消除,TS错误17→1)+前端build零错误+5新增测试+1569 scanner+51 backtest全通过 |
...
| vue-tsc总错误(MarketMonitor) | 17 | 1 | -94% |
| 前端build | ✅ | ✅ | 无变化 |
...
| TestMarketMonitorViewTSCleanup | 3 | 26个变量移除+scanTraceCode标注+weeklyReviewData解构 |
...
零。所有变更仅影响前端视图层script解构和版本常量, 后端/回测引擎零文件修改。
