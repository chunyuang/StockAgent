...
START_DATE = "20260105"
END_DATE = "20260320"
TARGET_STOCK_COUNT = 300  # 先下载300只

def get_akshare_stock_daily_ak_full(symbol, start_date, end_date):
    """
    使用正确的AKShare接口获取股票日线数据
    """
    try:
        # AKShare股票历史行情接口：stock_zh_a_hist
        # 接口文档：https://akshare.akfamily.xyz/data/stock/stock.html#id2
        ...
        # 四舍五入到两位小数
        df[['up_limit', 'down_limit', 'pre_close']] = df[['up_limit', 'down_limit', 'pre_close']].round(2)
...
def get_stock_list():
    ...
    try:
        ...
        # 提取股票代码，去掉前缀
        stock_zh_a_spot_df['code'] = stock_zh_a_spot_df['代码'].str.extract(r'(\d{6})')
        print(f"获取到 {len(stock_zh_a_spot_df)} 只A股股票")
...
def main():
    ...
    for code in tqdm(target_codes, desc="下载进度"):
        try:
            ...
            # 下载数据
            df = get_akshare_stock_daily_ak_full(code, START_DATE, END_DATE)
    ...
    print(f"唯一股票数: {unique_stocks} 只")
    print(f"唯一交易日数: {unique_dates} 天")
    
    # 检查字段完整性
    sample = collection.find_one({'pre_close': {'$exists': True}})
    if sample:
        print("\n字段完整性检查:")
    ...
    # 检查涨停股票
    print("\n📈 涨停股票检查:")
    dates = sorted(collection.distinct('trade_date'))[:5]  # 前5个交易日
