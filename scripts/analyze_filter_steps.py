...
def analyze_first_few_dates():
    ...
    # 获取前几个交易日
    dates_cursor = db.stock_daily_ak_full_ak.distinct("trade_date", {})
    ...
    for i, today_int in enumerate(dates[:20]):
        ...
        # 步骤1: 获取昨日所有股票
        prev_docs = list(db.stock_daily_ak_full_ak.find(
            {"trade_date": prev_int},
        ...
        # 步骤3: 检查这些股票今日开盘价
        today_docs = list(db.stock_daily_ak_full_ak.find(
            ...
            if limit_up_stocks:
                ...
                if sample in prev_data and sample in today_data:
                    ...
                    if diff >= 0:
                        ...
                        print("          开盘就涨停或高开，不满足策略条件")
                    else:
                        print(f"      ⚠️ 今日开盘价 < 昨日涨停价 (差: {diff:.2f})")
                        print("          满足价格条件，但可能被其他条件过滤")
