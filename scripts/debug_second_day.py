#!/usr/bin/env python3
"""Debug why zero signals - check second day 20260106"""
...
async def debug_second_day():
    print("=" * 60)
    print("Debug: 半路追涨 - 第二个交易日 20260106")
    print("=" * 60)
    ...
    if count_limit_up > 0:
        print("\n=== limit_up_yesterday == 1 AND open_below_limit == 1 统计 ===")
        ...
        if count_both > 0:
            ...
            for _, row in result.iterrows():
                print(f"  {row['ts_code']}: limit_up_yesterday={row['limit_up_yesterday']}, open_below_limit={row['open_below_limit']:.4f}, amount={row['amount']:.0f}")
    
    print("\n" + "=" * 60)

if __name__ == "__main__":
    asyncio.run(debug_second_day())
