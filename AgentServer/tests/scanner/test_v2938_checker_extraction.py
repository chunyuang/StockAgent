...
class TestRunCheckerOnPositions:
    ...
    def test_compare_mode_uses_extracted_method(self):
        """compare模式使用_run_checker_on_positions"""
        src = _read_source()
        lines = src.split("\n")
        in_compare = False
        found = False
        for line in lines:
            if "def _check_positions_compare" in line:
                ...
                break
            elif in_compare and "_run_checker_on_positions" in line:
                found = True
                break
        assert found, "_check_positions_compare应使用_run_checker_on_positions"
    ...
    def test_compare_mode_calls_persist(self):
        ...
        for line in lines:
            if "def _check_positions_compare" in line:
                ...
                break
            elif in_compare and "_persist_compare_diff" in line:
                found = True
                break
...
class TestCodeReduction:
    ...
    def test_position_checker_line_count(self):
        ...
        assert len(lines) < 700, f"position_checker.py行数{len(lines)}应<700"
...
class TestVersionSync:
    ...
    def test_design_doc_version(self):
        ...
        with open(api_path) as f:
            src = f.read()
        assert '_DESIGN_DOC_VERSION = "v2.9.38"' in src

    def test_docstring_mentions_v2938(self):
        """position_checker.py文档提到v2.9.38"""
        src = _read_source()
        assert "v2.9.38" in src
