...
class TestRunCheckerOnPositions:
    ...
    def test_compare_mode_uses_extracted_method(self):
        """compare模式使用_run_checker_on_positions(v2.9.74:通过_run_compare_both委托)"""
        src = _read_source()
        # v2.9.74重构: _check_positions_compare委托给_run_compare_both, 后者调用_run_checker_on_positions
...
class TestCompareDiffPersistence:
    ...
    def test_compare_mode_calls_persist(self):
        """compare模式调用_persist_compare_diff(v2.9.74:通过_handle_compare_diff委托)"""
        src = _read_source()
        # v2.9.74重构: _check_positions_compare委托给_handle_compare_diff, 后者调用_persist_compare_diff
...
class TestCodeReduction:
    ...
    def test_position_checker_line_count(self):
        ...
        assert len(lines) < 1000, f"position_checker.py行数{len(lines)}应<1000"  # v2.9.106: +47行风控审计trail
