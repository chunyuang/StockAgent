...
修复项:
1. 版本不同步: _get_version_info()写死v2.9.7 → 常量_DESIGN_DOC_VERSION=v2.9.9
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == "_DESIGN_DOC_VERSION":
                        if isinstance(node.value, ast.Constant):
                            assert node.value.value == "v2.9.9", \
                                f"_DESIGN_DOC_VERSION={node.value.value}, 期望v2.9.9"
                            found = True
