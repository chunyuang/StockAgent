...
class TestPendingSellsThreadSafety:
    ...
    def test_pending_sells_read_uses_state_lock(self):
        """读取pending_sells使用state_lock(v2.9.72: 拆分到_build_risk_metrics)"""
        source = _read_api_scanner()
        # v2.9.72: state_lock已拆到_build_risk_metrics子方法中
    ...
    def test_pending_sells_count_not_direct_getattr(self):
        """不应直接用getattr(scanner, '_pending_sells', {})无锁读取(v2.9.72: 已拆到_build_risk_metrics)"""
        source = _read_api_scanner()
