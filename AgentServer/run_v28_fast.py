#!/usr/bin/env python3
"""V28 baseline backtest - with logging disabled for speed"""
import asyncio, sys, os, time, logging
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
os.chdir(os.path.dirname(os.path.abspath(__file__)))

# Disable all logging before importing
...
async def main():
    ...
    if result and 'error' not in result:
        ...
        print(f"SELL: {srs}")
        # Analyze profit protection sells
        ...
        # Analyze冲高回落 sells
        chg_sells = [t2 for t2 in mt if t2.get('sell_reason','') == '冲高回落']
        print(f"冲高回落: {len(chg_sells)}笔")
        for s in chg_sells[:5]:
            print(f"  CHG: {s['ts_code']} {s['strategy']} {s['buy_date']}->{s['sell_date']} {s['profit_pct']:.2f}%")
    else:
