...
GLOBAL_RISK = {
    ...
    "dragon_head_early_exit_min_profit": 0.05,  # 【V69-P1:从0.04→0.05,龙头低吸5天利润<5%才提前退出;V67的4%阈值仍过早退出部分4-5%利润的龙头,5%给龙头更多兑现空间;龙头低吸93.5%胜率说明大部分持仓盈利,5%阈值更合理】
    "intraday_lock_min_high_rise": 0.06,  # 盘中利润锁定:冲高≥6%(V67:从5%→6%,5%对超短线来说偏低,3-5%的日内冲高很常见,过早锁定导致大量盈利股被退出;6%只在真正大冲高时触发,配合2%回撤阈值更精确)
    "intraday_lock_pullback_pct": 0.03,     # 盘中利润锁定:从高点回撤≥3%(V69:从2.5%→3%,2.5%在超短线中仍偏敏感,日内3%回撤以下属正常波动;V68基线32笔利润锁定中部分2.5-3%回撤的轻微波动被误触发;3%更精确减少误触发,让利润跑更远)
...
STRATEGY_CONFIGS = {
    ...
    "first_limit_up": {
        ...
        "params": {
            ...
            "hit_probability_slow": 0.45,                # 盘中板(开盘<2%)45%(V69:从55%→45%,首板4笔亏损平均-3.4%是最大拖累;55%成交过多低质量板,45%更保守减少亏损信号;V67未验证55%效果但数据表明首板贡献仅1%收益,收紧成交概率降低亏损概率)
