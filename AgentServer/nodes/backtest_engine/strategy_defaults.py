...
GLOBAL_RISK = {
    ...
    "dragon_head_early_exit_min_profit": 0.04,  # 【V67-P1:从0.03→0.04,龙头低吸5天利润<4%才提前退出;V64的3%阈值过早退出多笔盈利3-4%的交易,4%给龙头更多时间兑现逻辑】
    "intraday_lock_min_high_rise": 0.06,  # 盘中利润锁定:冲高≥6%(V67:从5%→6%,5%对超短线来说偏低,3-5%的日内冲高很常见,过早锁定导致大量盈利股被退出;6%只在真正大冲高时触发,配合2%回撤阈值更精确)
    "intraday_lock_pullback_pct": 0.025,    # 盘中利润锁定:从高点回撤≥2.5%(V68:从2%→2.5%,2%回撤在超短线中偏敏感,日内2%回撤很常见;2.5%减少误触发,只在真正大幅回撤时触发;V67基线34笔利润锁定中部分是2-3%回撤的轻微波动,2.5%更精确)
...
STRATEGY_CONFIGS = {
    ...
    "first_limit_up": {
        ...
        "params": {
            ...
            "hit_probability_slow": 0.55,                # 盘中板(开盘<2%)55%(V67:从50%→55%,首板贡献仅1%收益,适当放宽增加信号量;开盘<2%的板质量较高,55%更接近实际成交率)
