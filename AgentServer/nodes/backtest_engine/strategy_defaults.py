...
GLOBAL_RISK = {
    ...
    "dragon_head_early_exit_min_profit": 0.04,  # 【V67-P1:从0.03→0.04,龙头低吸5天利润<4%才提前退出;V64的3%阈值过早退出多笔盈利3-4%的交易,4%给龙头更多时间兑现逻辑;V69验证5%过宽导致-15%收益下降,4%仍是最优】
    "intraday_lock_min_high_rise": 0.06,  # 盘中利润锁定:冲高≥6%(V67:从5%→6%,5%对超短线来说偏低,3-5%的日内冲高很常见,过早锁定导致大量盈利股被退出;6%只在真正大冲高时触发,配合2%回撤阈值更精确)
...
STRATEGY_CONFIGS = {
    "halfway_chase": {
        ...
        "params": {
            ...
            "min_volume_ratio": 1.5,        # 量比≥1.5(V67:从2.0→1.5,2.0过严导致信号偏少;1.5在Q1数据中增加5-8笔交易且胜率>70%,信号量增加带来的收益>质量损失)
        ...
        "riskParams": {
            ...
            "hold_protection_threshold": 0.04,     # 【V71-P0-1:半路追涨持仓保护4%】低于全局6%,半路追涨选股不稳定(调仓卖出占26%),4%保护可让更多2-5%盈利股免于调仓卖出,由利润锁定/冲高回落在更优价位退出;6%过严导致6笔2-5%盈利股被调仓卖出(avg+3.5%),4%可保护这些股继续持有
    ...
    "first_limit_up": {
        ...
        "params": {
            ...
            "hit_probability_slow": 0.45,                # 盘中板(开盘<2%)45%(V70:从55%→45%,首板21笔胜率仅42.9%/12笔亏损,slow板55%过于乐观导致过多低质量成交;slow板质量不如fast/normal,45%更接近实际成交率;V69验证55%是最优但当时基线不同,V70首板止损率42.9%需要收紧成交)
