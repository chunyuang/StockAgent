    ...
    async def compute_factors(
        ...
        if settings.mode == "backtest":
            ...
            for name in factor_names:
                projection[name] = 1
            # 【Phase2】确保high/open/pre_close也在projection中，用于计算盘中可观测因子
            for _field in ["high", "open", "pre_close"]:
                if _field not in projection:
                    projection[_field] = 1
        ...
        if "daily" in data_sources:
            ...
            all_result = await mongo_manager.find_many(
                ...
                projection={
                    ...
                    "first_limit_up": 1, "limit_up_yesterday": 1, "limit_up_count": 1, "is_limit_up": 1, "is_limit_down": 1,
