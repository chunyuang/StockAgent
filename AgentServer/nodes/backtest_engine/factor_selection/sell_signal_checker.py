...
SLIPPAGE_RULES = {
    ...
    '利润锁定': False,      # 【V70-P0-1:利润锁定不扣滑点】盘中冲高已从高点回撤≥2.5%,以close卖出是最保守估计,再扣0.2%滑点导致利润锁定实际盈利被侵蚀(基线45笔利润锁定×0.2%=~0.9%累计额外成本);与止盈不扣滑点同理——精确价格触达已含保守估计
    '高开即卖': True,      # 主动保护:以open卖出,扣滑点
    '止盈':     False,     # 【V49-P0-4修复:止盈不扣滑点——止盈以精确价格触达卖出,已含保守估计,额外滑点导致止盈实际几乎不盈利】
    '止损':     False,     # 被迫卖出:止损价已含保守估计,不额外扣
    '跳空止损': False,     # 被迫卖出:open直接跳空低开,已是最差情况
    '追踪止损': False,     # 【V76-P1-5:追踪止损不扣滑点——与利润锁定同理,从峰值回撤≥阈值以close卖出,已含保守估计】
    '超时':     False,     # 被迫卖出:超时强卖,不应再惩罚
    '强制空仓': False,     # 被迫卖出:open价卖出已是最差情况
    '停牌超时强卖': False, # 被迫卖出:被迫以最后有效价卖出
    '龙头5天低利润': False, # 【V60:龙头低吸5天利润<3%提前退出,类似超时强卖,不扣滑点】
    '调仓卖出': True,      # 正常调仓:扣滑点
    '减仓':     True,      # 正常调仓:扣滑点
}
...
def check_intraday_profit_lock(holding, market_data, params):
    ...
    min_high_rise = params.get('intraday_lock_min_high_rise', GLOBAL_RISK.get('intraday_lock_min_high_rise'))  # 盘中冲高阈值(V59:从4%→5%)
    pullback_pct = params.get('intraday_lock_pullback_pct', GLOBAL_RISK.get('intraday_lock_pullback_pct'))     # 从高点回撤阈值(V59:从1.5%→2%)
    min_profit = params.get('intraday_lock_min_profit', GLOBAL_RISK.get('intraday_lock_min_profit'))         # 收盘最低利润阈值
...
STRATEGY_PULLBACK_PARAMS = {
    '半路追涨': {
        ...
        'pullback_profit_lock_threshold': 0.08,  # V65:半路追涨利润≥8%时不触发冲高回落,让利润锁定/超时处理;与strategy_defaults对齐;V72验证6%回退-3.36%
    },
    '跌停翘板': {
        'pullback_high_threshold': 0.05,
