...
class SignalManager:
    ...
    def _check_signal_eligibility(self, sig: ScanSignal) -> tuple:
        ...
        #   现在: min(dynamic, MAX_POSITIONS) 确保永不超10只
        dynamic_max = scanner._get_dynamic_max_positions()
        MAX_POSITIONS = min(dynamic_max, scanner.MAX_POSITIONS)
    ...
    def _check_strategy_params_filter(self, sig) -> tuple:
        ...
        if strategy == 'dragon_head':
            abs(getattr(sig, 'pct_chg', 0) or 0)
        ...
        if ma5 > 0 and current_price > 0:
            ...
            # 【v2.9.124】高价股(>100元)收紧: 偏离MA5>5%不追(高价股波动大追高风险)
            # 低价股保持: 低于MA5超过2%不追(弱势不追)
            if current_price > 100:
                ma5_max_dev = 5.0
                if ma5_diff_pct > ma5_max_dev:
                    reason = (
                        f"MA5超涨·高价股{current_price:.2f}>MA5{ma5:.2f} "
                        f"偏离{ma5_diff_pct:+.1f}%>+{ma5_max_dev:.0f}% (追高风险)"
                    )
                    sig.signal_status = "blocked"
                    sig.layer_trace = sig.layer_trace or {}
                    sig.layer_trace["execution"] = {"mode": "strategy_filter", "reason": reason}
