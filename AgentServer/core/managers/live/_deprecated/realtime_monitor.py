...
class RealTimeMonitor:
    ...
    async def _check_positions(self) -> List[Dict]:
        ...
        try:
            from core.managers.live.position_manager import PositionManager
        ...
        try:
            ...
            for target in self.config["target_stocks"]:
                ...
                if isinstance(target, dict):
                    ...
                    if target_price > 0 and current_price <= target_price * 1.01:
                        alert_key = f"target_buy_{code}"
                        if self._should_alert(alert_key):
                            alerts.append({
                                "level": "info",
                                "title": f"🎯 目标股提醒：{name}",
                                "content": f"{name}({code}) 当前价{current_price:.2f}，接近目标买入价{target_price:.2f}，策略：{strategy}"
                            })
                else:
                    # 简单提醒达到预设的涨幅区间
                    if 0.5 <= pct_chg <= 3:
