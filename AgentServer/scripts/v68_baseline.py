...
async def main():
    ...
    task_id = f"v68_baseline_{int(time.time())}"
    logs = []
    ...
    print(f"🚀 V68审查基线 | {inner_params['start_date']}~{inner_params['end_date']} | 4策略默认参数")
    t0 = time.time()
    ...
    if result.get("error"):
        ...
        with open(os.path.join(BASE, 'V68_baseline_error.json'), 'w') as f:
    ...
    print(f"\n{'='*65}")
    print(f"📊 V68审查基线结果 ({elapsed:.1f}s)")
    print(f"{'='*65}")
    print(f"  总收益:     {tr:.2f}%")
    ...
    # Save full result
    with open(os.path.join(BASE, 'V68_baseline_result.json'), 'w') as f:
        json.dump(result, f, default=str, indent=2, ensure_ascii=False)
    
    # Save summary
    with open(os.path.join(BASE, 'V68_baseline_summary.json'), 'w') as f:
        json.dump({
