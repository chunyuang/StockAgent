...
async def main():
    ...
    task_id = f"v67_full_{int(time.time())}"
    logs = []
    
    async def push_log(tid, msg):
        logs.append(msg)
        if any(k in msg for k in ['📊', '🎯', '收益', 'Alpha', '胜率', '盈亏', '回撤', '夏普', '索提', '交易笔', '策略收益', '❌']):
            print(msg)
    ...
    print(f"🚀 V67全量2年回测 | {inner_params['start_date']}~{inner_params['end_date']}")
    t0 = time.time()
    ...
    print(f"\n{'='*65}")
    print(f"📊 V67全量2年结果 ({elapsed:.1f}s)")
    print(f"{'='*65}")
    ...
    with open(os.path.join(BASE, 'V67_full_results.json'), 'w') as f:
        json.dump({
