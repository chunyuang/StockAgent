...
async def main():
    ...
    task_id = f"v73_p0v3_{int(time.time())}"
    logs = []
    ...
    print(f"🚀 V73-P0v3 (slow 0.65 + 半路hold_prot 3%) | {inner_params['start_date']}~{inner_params['end_date']}")
    t0 = time.time()
    ...
    print(f"\n{'='*65}")
    print(f"📊 V73-P0v3 结果 ({elapsed:.1f}s)")
    print(f"{'='*65}")
    print(f"  总收益:     {tr:.2f}%")
    print(f"  最大回撤:   {md:.2f}%")
    print(f"  夏普:       {sr:.2f}")
