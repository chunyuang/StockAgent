"""V69最终回测 - 仅保留正向优化(龙头低吸SL 3%+利润锁定回撤2.5%)"""
...
async def main():
    ...
    task_id = f"v69_final_{int(time.time())}"
    logs = []
    ...
    print(f"🚀 V69最终回测 | {inner_params['start_date']}~{inner_params['end_date']}")
    t0 = time.time()
    ...
    # V68 baseline comparison
    print(f"\n{'='*65}")
    print(f"📊 V68→V69 最终对比")
    print(f"{'='*65}")
    v68 = {'tr': 315.52, 'md': 3.46, 'sr': 14.14, 'wr': 83.1, 'plr': 3.13, 'tt': 118}
    v68_sr = {'跌停翘板': (317.80, 10.30, 3.42), '龙头低吸': (307.89, 3.16, 3.43), '半路追涨': (162.43, 3.67, 2.58), '首板打板': (85.51, 5.58, 2.51)}
    ...
    for sname, (v68_tr, v68_dd, v68_plr) in v68_sr.items():
        sdata = strategy_results.get(sname, {})
        s_tr = sdata.get('total_return', 0)
        s_dd = sdata.get('max_drawdown', 0)
        s_plr = sdata.get('profit_loss_ratio', 0)
        print(f"  [{sname}] 收益{v68_tr:.2f}→{s_tr:.2f}({s_tr-v68_tr:+.2f}) 回撤{v68_dd:.2f}→{s_dd:.2f}({s_dd-v68_dd:+.2f})")
    
    with open(os.path.join(BASE, 'V69_final_result.json'), 'w') as f:
