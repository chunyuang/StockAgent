"""V67基线Q1回测 - 使用strategy_defaults.py默认参数(与V66一致)"""
...
async def main():
    ...
    task_id = f"v67_default_{int(time.time())}"
    logs = []
    
    async def push_log(tid, msg):
        logs.append(msg)
        if any(k in msg for k in ['📊', '🎯', '收益', 'Alpha', '胜率', '盈亏', '回撤', '夏普', '索提', '交易笔', '回测完成', '❌']):
            print(msg)
    
    # Use strategy_defaults.py defaults (no overrides) - same as V66
    inner_params = {
        "start_date": "20250105",
        "end_date": "20250331",
        ...
        "selected_strategies": ALL_STRATEGIES,  # Use defaults from strategy_defaults.py
        "params": {
    ...
    print(f"🚀 V67基线(默认参数)Q1回测 | {inner_params['start_date']}~{inner_params['end_date']}")
    t0 = time.time()
    ...
    print(f"\n{'='*65}")
    print(f"📊 V67基线(默认参数)Q1结果 ({elapsed:.1f}s)")
    print(f"{'='*65}")
    ...
    # Save
    with open(os.path.join(BASE, 'V67_baseline_default_q1.json'), 'w') as f:
        json.dump({
