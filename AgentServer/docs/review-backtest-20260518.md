...
2. 9层筛选管道: 生效正常
3. 卖出原因统计: 6类分类工作正常(other从3→0)
4. 参数扫描: sweep API可用
5. 历史记录: 增删改查正常,task_type字段正确写入
6. WebSocket/轮询: 双通道回退正常
7. 策略对比: 多策略KPI对比正常
8. 净值曲线: 归一化正确(1.0起始),前端不再二次归一化
9. 日收益图: daily_profit已归一化,直接×100显示%

## 已修复汇总 (commit 536dc91 + 7484dbe)
| ID | 问题 | 修复 |
|----|------|------|
| P0-1 | 净值曲线二次归一化 | 前端net_value不再÷initialCash |
| P0-2 | 日收益单位错误 | daily_profit已归一化,直接×100 |
| P1-1 | 历史记录task_type缺失 | 提交时写入ultra_short |
| P1-3 | 卖出原因other=3 | 扩展匹配+跳过未平仓交易,other=0 |
| P1-5 | 雷达图max硬编码 | 动态计算max值 |
| P2-1 | 页面标题过时 | 更新描述 |
| P2-2 | KPI缺盈亏比 | 新增盈亏比chip |
| P2-3 | 交易表缺金额 | 新增盈亏额列 |
| UI | 卖出原因条粗糙 | 彩色圆点+药丸样式+条件显示 |
| UI | KPI卡片单调 | 渐变背景+hover阴影 |
