...
async def run_full_market_backtest():
    ...
    print(f"回测区间: 20260105 ~ 20260320")
    print(f"初始资金: 1,000,000 元")
    print(f"模拟参数: 滑点千1、佣金万2、印花税千1、涨停无法买入、跌停无法卖出")
    print(f"仓位限制: 单票最大20%、总仓位最大100%")
    print("="*80)
    ...
    # 验证数据
    db = mongo_manager.db
    stocks = await db.stock_daily.distinct("ts_code")
    dates = sorted(await db.stock_daily.distinct("trade_date"))
    start_date = "20260105"
    end_date = "20260320"
    
    # 筛选回测区间内的日期
    test_dates = [d for d in dates if str(d) >= start_date and str(d) <= end_date]
    total_records = await db.stock_daily.count_documents({
